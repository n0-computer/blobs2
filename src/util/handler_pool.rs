...
type BoxedHandler = Box<dyn FnOnce(&ConnectResult) -> n0_future::boxed::BoxFuture<anyhow::Result<()>> + Send + 'static>;
...
async fn run_connection_actor(
    ...
    // Connect to the node
    let mut connection = match endpoint
    ...
    if !matches!(connection, ConnectResult::Connected(_)) {
            ...
            handler = rx.recv() => {
                match handler {
                    Some(handler) => {
                        ...
                        tasks.spawn(handler(&connection));
                    }
                    None => {
            ...
            task_result = tasks.join_next(), if !tasks.is_empty() => {
                match task_result {
                    ...
                    Some(Ok(Err(e))) => {
                        ...
                        connection = ConnectResult::ConnectionError(e);
                        let _ = main_tx.send(ActorMessage::ConnectionShutdown { id: node_id }).await;
                    }
                    Some(Err(e)) => {
                        tracing::error!("Task panicked for node {}: {}", node_id, e);
                        connection = ConnectResult::JoinError(e);
                        let _ = main_tx.send(ActorMessage::ConnectionShutdown { id: node_id }).await;
                    }
                    None => {
    ...
    if let ConnectResult::Connected(connection) = &connection {
            ...
            match msg {
                ActorMessage::Handle { id, mut handler } => {
                    ...
                    if let Some(conn_tx) = self.connections.get(&id) {
                        if let Err(tokio::sync::mpsc::error::SendError(e)) = conn_tx.send(handler).await {
                            handler = e;
                        } else {
                            continue;
                        }
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = run_connection_actor(endpoint, id, conn_rx, main_tx, options).await {
                    ...
                    if conn_tx.send(handler).await.is_err() {
                        tracing::error!("Failed to send handler to new connection actor for {}", id);
                        self.connections.remove(&id);
                    }
                }
